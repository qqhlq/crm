<template>
  <el-form :model="xiaomiAccount"  class="w-form" label-width="72px" :label-position="screenWidth <= 769 ? 'top' : 'right'">
    <el-form-item class="w-form__item" label="监控链接" prop="link">
      <span class="edit-monitor">
        <span :class="[
          'edit-monitor__url',
          xiaomiAccount.clickUrl === null || xiaomiAccount.clickUrl === '' ? 'edit-monitor__url--empty' : ''
        ]">
          {{ xiaomiAccount.clickUrl === null || xiaomiAccount.clickUrl === '' ? '未填写' : '已填写'  }}
        </span>
        <el-button type="text" class="edit-monitor__btn" @click="editMonitorUrl(xiaomiAccount.clickUrl)">
    ...
    <el-form-item class="w-form__item" label="上报类型" prop="reportType">
      <el-radio-group v-model="xiaomiAccount.reportType" @input="changeReportType">
  ...
  export default {
    ...
    computed: {
      ...mapGetters({
        ...
        xiaomiAccount: 'deliveryMonitor/xiaomiAccount'
      })
    },
    methods: {
      ...mapActions({
      ...
      async changeReportType(reportType) {
        ...
        let data = await self.updateReportType({
          platformId: 2,
          customId: this.$route.params.cid,
          reportType: self.xiaomiAccount.reportType
        })
        if (data.data === true) {
